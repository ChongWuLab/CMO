...
enhancer = readRDS("processed_gene_CpG_inf.rds")

enhancer = enhancer[enhancer[,"chromosome_name"]==chr.id,]
...
for(i in 1:dim(enhancer)[1]) {
    ...
    weights.dir = paste("gene/",enhancer[i,"ensembl_gene_id"],".RData",sep="")
...
saveRDS(enhancer,paste("processed_gene_CpG_mapped_CHR",chr.id,".rds",sep=""))


final.dat = NULL
for(chr.id in 1:22) {
    tmp = readRDS(paste("processed_gene_CpG_mapped_CHR",chr.id,".rds",sep=""))
    final.dat = rbind(final.dat,tmp)
}

saveRDS(final.dat,"processed_gene_CpG_mapped_inf.rds")
