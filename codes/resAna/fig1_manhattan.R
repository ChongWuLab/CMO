...
gg.manhattan <- function(x, chr = "CHR", bp = "BP", p = "P", snp = "SNP", annot = "annot", trait = "trait",
  ...
  if (nchr == 1) {
    ...
    plot <- plot + geom_text_repel(data = subset(d, ANNOT == 1), aes(label = SNP), segment.color = "#3d3d3d", size = 5, arrow = arrow(length = unit(0.05, "npc")))
  } else {
    ...
    plot <- plot + geom_text_repel(data = subset(d, ANNOT == 1), aes(label = SNP), segment.color = "#636363", size = 5, arrow = arrow(length = unit(0.05, "npc")))
...
crosRes = crosRes[!is.na(crosRes[, "Omni_ACAT_MultiXcan"]),]

crosRes[, "P0"] = floor((crosRes[, "P0"] + crosRes[, "P1"]) / 2)
crosRes = crosRes[, c("CHR", "Gene", "P0", "Omni_ACAT_MultiXcan")]

tmp = crosRes[crosRes[, 4] < 2.1e-6,]
tmp.gene = tmp[, 2]
tmp = single[single[, 4] %in% tmp[, 2],]
tmp = tmp[tmp[, "p-ACAT"] < 1e-4,]
sig.gene = unique(tmp[, 4])
...
crosRes[crosRes[, 4] < 1e-20, 4] = 1e-20
annot.gene = annot[, 2]
annot.gene = annot.gene[1:8]
annot.gene = c(annot.gene, "APOE")
crosRes$annot = as.numeric(crosRes[, 2] %in% annot.gene)

plot = gg.manhattan(crosRes, cols = c("#3B77AE", "#CD342B"), suggestiveline = -log10(sig_level), p = "p-value", plot_title = "")
ggsave("fig1_manhattan.pdf", plot)