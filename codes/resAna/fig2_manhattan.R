...
logp = TRUE) {
    ...
    if (nchr == 1) {
        ...
        plot <- plot + geom_text_repel(data = subset(d, ANNOT == 1), aes(label = SNP), segment.color = "#3d3d3d", size = 5, arrow = arrow(length = unit(0.05, "npc")) ,    nudge_y      = 0.05,
        direction    = "y")
    } else {
        ...
        plot <- plot + geom_text_repel(data = subset(d, ANNOT == 1), aes(label = SNP), segment.color = "#636363", size = 5, arrow = arrow(length = unit(0.05, "npc")),nudge_y      = 0.05,
        direction    = "y")
...
crosRes = crosRes[, c("CHR", "geneID", "P0", "cross_ACAT")]

tmp = crosRes[crosRes[, 4] < 2.9e-6,]
tmp.gene = tmp[, 2]
...
annot.gene = annot.gene[!annot.gene %in% c("CREBZF","MIER2","MARK4","BCL3","CNN2")]
